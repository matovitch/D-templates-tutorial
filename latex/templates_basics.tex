...
Une template est une recette, un modèle qui va engendrer du code à la compilation selon des paramètres que vous lui donnerez. Les templates sont du \emph{code paramétré}. Chaque définition est écrite une seule fois dans un module et peut être intanciée plusieurs fois avec des paramètres différents pour produire un code qui dépendra des paramètres que vous utiliserez.
...
Voici la syntaxe pour déclarer une template :
...
\index{syntax!template declaration}
\begin{dcode}
template nomDeTemplate(liste, de, parametres)
{
  // Declarations syntaxiquement correctes ici.
  // Les parametres sont accessible dans ce bloc.
}
\end{dcode}

\DD{NomDeTemplate} est un simple identifiant en code D et la liste de paramètres template est séparée par des virgules et de longueur zéro ou plus. 
